...
int getWinner(void)
{
    ...
    for(i=0;i<N_PLAYER;i++)
    {
    	if(player_status[i] != PLAYERSTATUS_DIE)
    	{
			...
			{
				if(player_finish[i] < player_finish[winner])
					winner = i;
			}
		}
	}
...
// ----- EX. 4 : player ------------
        ...
        //step 2-4. coin
        coinPos = board_getBoardCoin(player_position[turn]);
        player_coin[turn] += coinPos;
        printf("%s gets %i coins!\n", player_name[turn], coinPos);
